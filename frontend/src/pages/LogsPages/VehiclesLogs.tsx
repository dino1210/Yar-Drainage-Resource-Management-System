...
export default function VehiclesLogs() {
  const [logs, setLogs] = useState<any[]>([]); // State to hold vehicle logs
  ...
  useEffect(() => {
    // Fetch vehicle logs from backend on component mount
    const fetchVehicleLogs = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/vehicle-logs`); // API URL for vehicles
        setLogs(response.data.vehicleLogs);  // Set fetched logs to state
        setLoading(false);
      } catch {
        setError("Error fetching vehicle logs");  // Set error state if fetch fails
        setLoading(false);
      }
    };

    fetchVehicleLogs();  // Call the function to fetch logs
  ...
  return (
    <div>
      <PageMeta title="Resource Logs: Vehicles Logs" description="" />
                ...
                <tbody>
                  {filteredLogs.map((log, index) => (
                    <tr key={index} className="hover:bg-gray-100 dark:hover:bg-gray-700 border-b-[1px] border-gray-200 dark:border-gray-700">
                      ...
                      <td className="px-4 py-4 text-center text-xs text-gray-700 dark:text-gray-300">{log.vehicle_name}</td>
